<template>
  <div class="card">
    ...
    <div class="card__right">
      ...
      <butler-button v-else v-on:click.native="confirme">开始返程</butler-button>
    </div>
  </div>
</template>

<style lang="scss" scoped>
@import './charge-returning.scss';
</style>
<script>
  ...
  methods: {
    confirme() {
      MessageBox.confirm('是否出发取车').then(action => {
        Api.putStatus({ status: 'BUTLER_RETURNING_CAR' }).then(res => {
          this.$store.dispatch({
            type: 'getOrder'
          });
        });
